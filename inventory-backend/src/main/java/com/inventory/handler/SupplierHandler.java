...
public class SupplierHandler {
    ...
    public boolean addSupplier(Supplier supplier) {
        String sql = "INSERT INTO suppliers (name, contact_info, email, address) VALUES (?, ?, ?, ?)";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (affectedRows > 0) {
                ...
                logger.info("Supplier '{}' added successfully with ID {}", supplier.getName(), supplier.getSupplierId());
                return true;
            }
    ...
    public Supplier getSupplierById(int supplierId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            logger.error("Error fetching supplier with ID {}", supplierId, e);
        }
    ...
    public boolean updateSupplier(Supplier supplier) {
        String sql = "UPDATE suppliers SET name = ?, contact_info = ?, email = ?, address = ? WHERE supplier_id = ?";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (affectedRows > 0) {
                logger.info("Supplier '{}' (ID: {}) updated successfully", supplier.getName(), supplier.getSupplierId());
                return true;
            }
    ...
    public boolean deleteSupplier(int supplierId) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            logger.error("Error deleting supplier with ID {}", supplierId, e);
            return false;
        }
    }
    ...
    public List<Supplier> searchSuppliersByName(String searchTerm) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            logger.info("Found {} suppliers matching '{}'", suppliers.size(), searchTerm);

        } catch (SQLException e) {
            logger.error("Error searching suppliers by name '{}'", searchTerm, e);
        }
     ...
     * @return Supplier name and number of products they supply
     */
    public List<String> getSuppliersWithProductCount() {
        List<String> results = new ArrayList<>();
        String sql = "SELECT s.supplier_id, s.name, COUNT(p.product_id) as product_count " +
                "FROM suppliers s LEFT JOIN products p ON s.supplier_id = p.supplier_id " +
                "GROUP BY s.supplier_id, s.name ORDER BY product_count DESC, s.name";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                String result = String.format("Supplier: %s (ID: %d) - Products: %d",
                        rs.getString("name"),
            ...
            logger.info("Retrieved supplier statistics for {} suppliers", results.size());

        } catch (SQLException e) {
            logger.error("Error fetching suppliers with product count", e);
        }
    ...
    public static void main(String[] args) {
        ...
        // Test 3: Add new supplier
        System.out.println("\n3. Adding new supplier:");
        Supplier newSupplier = new Supplier("Test Supplier Ltd", "+998 90 999 8877",
                "test@supplier.com", "Tashkent, Test Street 123");
        boolean added = handler.addSupplier(newSupplier);
        System.out.println(added ? "✓ Supplier added: " + newSupplier : "✗ Failed to add supplier");

        // Test 4: Search suppliers
        System.out.println("\n4. Searching for 'Coca':");
        List<Supplier> searchResults = handler.searchSuppliersByName("Coca");
        ...
        if (newSupplier.getSupplierId() > 0) {
            ...
            System.out.println(updated ? "✓ Supplier updated" : "✗ Update failed");

            // Verify update
            Supplier updatedSupplier = handler.getSupplierById(newSupplier.getSupplierId());
            System.out.println("Updated: " + updatedSupplier);

            // Test 6: Delete supplier
            System.out.println("\n6. Deleting test supplier:");
            boolean deleted = handler.deleteSupplier(newSupplier.getSupplierId());
            System.out.println(deleted ? "✓ Supplier deleted" : "✗ Deletion failed");
        }

        // Test 7: Get suppliers with product count
        System.out.println("\n7. Suppliers with product counts:");
        List<String> stats = handler.getSuppliersWithProductCount();
